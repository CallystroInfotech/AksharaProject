...
public class StartGameActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       /* if(Utils.checkOldDirectoryExists.exists() && Utils.checkOldDirectoryExists.isDirectory())
        {
            String[] children = Utils.checkOldDirectoryExists.list();
            for (int i = 0; i < children.length; i++)
            {
                new File(Utils.checkOldDirectoryExists, children[i]).delete();
            }
        }*/
    ...
    private void checkForTheFilesDownloaded(String lang)
    {
      /*  switch(lang)
        {
            case "English" :if(!Utils.checkForSoundFileEnglish.exists())
            {
                Toast.makeText(this, "English Sounds Does not exist", Toast.LENGTH_LONG).show();
                new DownloadEnglishSoundFiles(this,Utils.downloadEnglishSoundsUrl);
                return false;
            }
                break;
            case "Hindi" : if(!Utils.checkForSoundFileHindi.exists())
            {
                Toast.makeText(this, "Hindi Sounds Does not exist", Toast.LENGTH_LONG).show();
                new DownloadHindiSoundFiles(this,Utils.downloadHindiSoundsUrl);
                return false;
            }
                break;
            case "Kannada" : if(!Utils.checkForSoundFileKannada.exists())
            {
                Toast.makeText(this, "Kannada Sounds Does not exist", Toast.LENGTH_LONG).show();
                new DownloadKannadaSoundFiles(this,Utils.downloadKannadaSoundsUrl);
                return false;
            }
                break;
            case "Odiya" : if(!Utils.checkForSoundFileOdiya.exists())
            {
                Toast.makeText(this, "Odiya Sounds Does not exist", Toast.LENGTH_LONG).show();
                new DownloadOdiyaSoundFiles(this,Utils.downloadOdiyaSoundsUrl);
                return false;
            }
                break;
            default: if(!Utils.checkForgameAssets.exists()||!Utils.checkForgameJson.exists())
            {
                Log.d("Download Assets","Download Assets");
                new DownloadAssetsFiles1(this,Utils.downloadAssetsUrl);
            }
            break;
        }*/
    ...
    private Boolean checkForTheFilesDownloadedExists(String lang)
    {
        if(lang.equals("English") && !Utils.checkForSoundFileEnglish.exists())
        {
            return false;
    ...
    void onStartGameClick() {
        ...
        /*boolean flag = checkForTheFilesDownloadedExists(user.getLanguage());
